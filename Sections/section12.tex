...
\section{Диагонализируемость}
    ...
    Пусть $\phi: \ V \to V$ - линейный оператор, $\dim V = n$, тогда в некотором базисе $V$, $\phi$ действует матрицей $A_\phi: \ Y = A_{\phi}\cdot X$, где $X\in \R^n$, а $Y$ - столбец образа этого вектора ($y = \phi(x)$). Пусть $\lambda = \alpha+i\beta$ ($\beta\neq0$) - корень характеристического многочлена.
