...
В ортогональном базисе $G_{\{ e'_1,...,e'_n\}} = \begin{pmatrix} |e'_1|^2 &\null&0 \\ \null&\ddots&\null \\ 0&\null&|e'_n|^2 \end{pmatrix} \Rightarrow \\ \Rightarrow |G_{\{ e'_1,...,e'_n\}}| = |e_1|^2\cdot...\cdot|e_n|^2$
...
\begin{definition}
    \begin{enumerate}
        ...
        $$\forall x,y \in \mathcal{E}: \ (\phi(x), y) = (x, \phi^{*}(y)) \ \ (1)$$
        \item Оператор $\phi$ - самосопряжённый, если
        $$\phi^{*} = \phi \Rightarrow \forall x,y \in \mathcal{E}: \ (\phi(x), y) = (x, \phi(y)) \ \ (2)$$
        \item Оператор $\phi$ - ортогональный, если
        $$\forall x,y \in \mathcal{E}: \ (\phi(x), \phi(y)) = (x, y) (3)$$.
        В частности, для ортогонального $\phi: \forall x \in \mathcal{E} \ \ |\phi(x)| = |x|$.
    \end{enumerate}
\end{definition}
\textbf{Условия (1)-(3) через матрицу Грама}\\
    Пусть в $\mathcal{E}$ зафиксирован базис $e = (e_1,...,e_n) \ (\dim \mathcal{E} = n)$. Пусть $x = eX, y = eY, G_e = ((e_i, e_j))$ - матрица Грама базиса $e$, $A_{\phi}$ - матрица $\phi$ в базисе $e$.\\
    (1): $\forall X, Y \in \R^n$:
...
\begin{proof}
    \begin{enumerate}
        ...
        Т.к. размерности равны и $\textup{Im}\phi \subseteq (\textup{Ker}\phi^{*})^{\perp}$, $\textup{Ker}\phi^{*} = (\textup{Im}\phi)^{\perp}$.
