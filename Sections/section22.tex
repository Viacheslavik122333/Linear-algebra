...
\end{proof} 
\subsection*{Ортогональные преобразования (движения или изометрии)}
\begin{definition}
...
\begin{proof}
    ...
    \[\Psi(b)=(t_{-u}\Phi)(b)\]
    \begin{multline*}
        \Psi(a+\widetilde{w})=t_{-u}(\Psi(a)+\phi(\widetilde{w}))=t_{-u}(a+v+\phi(\widetilde{w}))=\\
...
неподвижны, а $\Psi$ определяется своим действием в гиперплоскости ортогональной к этой прямой:
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{cases}
        \]
        \item Композиция параллельного переноса в доль оси и симметрии относительно оси:
        \[
        \begin{cases}
            x'=x+a,\\
            y'=-y+b
        ...
        \end{cases}
        \]
        Согласно общей теореме, существует неподвижаня точка, что после переноса в эту точку остается только поворот.
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{cases}
        \]
        что является композицией ортогональной симметрии относительно плоскости $\xi=\eta=0$ и параллельного переноса на вектор $(a,b,0)$, параллельно этой плоскости.
        \item
        \[
        \begin{cases}
            x'=x\cos{\alpha}-y\sin{\alpha}+a,\\
        ...
        \end{cases}
        \]
        что является композицией поворота вокруг прямой, параллельной $(0,0,1)$ на угол $\alpha$ и переноса на вектор $(0,0,c)$ вдоль этой прямой (винтовое движение).
        \item
        \[\begin{cases}
            x'=x\cos{\alpha}-y\sin{\alpha}+a,\\
            y'=x\sin{\alpha}+y\cos{\alpha}+b,\\
        ...
        \end{cases}
        \]
        что является композицией симметрии относительно плоскости $\zeta=c$, повотора вокруг прямойЮ перпендикулярной этой плоскости, и параллельного переноса на вектор $(0,0,c)$, который параллелен этой плоскости.
    \end{enumerate}
\end{enumerate}
