...
\section{Корневые подпространства}
    ...
    \begin{proof}\tab 
        \begin{enumerate}
            ...
            Обратно: пусть $y_i \in \text{Ker}(\phi-\lambda_i \cdot \text{id})$. Знаем, что $y_i = Q_1(y_i) + ... + Q_s(y_i)$, причём в $Q_j$ при $j \neq i$ содержится множитель $(\phi-\lambda_i \cdot \text{id})$. Отсюда $Q_j(y_i) = 0$ при $j \neq i$, т.е. $y_i = Q_i(y_i) \Rightarrow y_i \in K_i \Rightarrow \text{Ker}(\phi-\lambda_i \cdot \text{id}) \subseteq K_i$.
        \end{enumerate}
    \end{proof} 
    \begin{theorem}
