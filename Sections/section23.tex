...
Отсюда:
\[f = A_{i_1,...,i_p}^{j_1,...,j_q}\cdot x_1^{i_1}\cdot...\cdot x_p^{i_p} \cdot u_{j_1}^1\cdot...\cdot u_{j_q}^q\]
...
\begin{definition}
    ...
    \[f(x_{\sigma(1)},...,x_{\sigma(p)}) = f(x_1,...,x_p) \eqno(1)\]
    Аналогично, если $g \in T_0^q(V)$, то $g$ - симметрический, если $\forall u^1,...,u^q \in V^*, \sigma \in S_q$
    \[f(u^{\sigma(1)},...,u^{\sigma(q)}) = f(u^1,...,u^q) \eqno(1')\] 
    Тензор $f \in T_p^0(V)$ $(\textup{char} F \neq 2)$ - кососимметрический, если $\forall x_1,...,x_p \in V, \sigma \in S_p$
    \[f(x_{\sigma(1)},...,x_{\sigma(p)}) = \sgn(\sigma)f(x_1,...,x_p) \eqno(2)\]
...
Определим $T(V) = \circledplus \limits_{q = 0}^\infty T_0^q(V)$ - множество финитных последовательностей тензоров $(f_0,...,f_s, 0,...)$. 
...
\begin{enumerate}
    ...
    Свойства:
    \begin{enumerate}
        \item $\textup{Sym}: T_p^0(V) \rightarrow T_p^0(V)$ - линейное отображение, $\textup{Im}\hspace{0.07cm}\textup{Sym} = T_p^+(V)$;
    ...
    \end{multline*}
     Свойства:
    \begin{enumerate}
        \item $\textup{Alt}: T_p^0(V) \rightarrow T_p^0(V)$ - линейное отображение, $\textup{Im}\hspace{0.07cm}\textup{Alt} = \Lambda_p$
...
Очевидно, что существует только одномерное подпространство, содержащее $x_1\wedge...\wedge x_n \forall x_i \in V$, т.е. $\dim \Lambda^1(V) = n$.\\
...
\begin{definition}
    Скалярное произведение - тензор типа (2, 0): 
    \[(x, y) = g_{ij}x^ix^j\] 
    $g_{ij}$ - метрический тензор.\\
...
- изоморфизм между $V$ и $V^*$. Аналогично можно рассмотреть свёртку $g^{ij}a_j = y^i$ - индекс поднимается наверх. Эти операции, очевидно, взаимно обратны: 
\[g_{ij}(g^{ij}a_j) = (g_{ij}g^{ij})a_j = a_j\]
